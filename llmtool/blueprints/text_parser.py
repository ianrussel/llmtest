...
def process():
    ...
    if len(df_string) > 5000:
        return jsonify({"message": "Text exceed max length of 1500 characters", "data": None, "error": True})
    if len(df_string) < 30:
        return jsonify({"message": "Text too short, min length is 30 characters", "data": None, "error": True})
